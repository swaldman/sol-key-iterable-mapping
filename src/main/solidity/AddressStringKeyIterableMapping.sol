...
library AddressStringKeyIterableMapping {
  struct Record {
  ...
  function get( Store storage store, address key ) internal view returns ( bool exists, string memory value ) {
    Record storage rec = store.map[key];
    exists = rec.exists;
    value = rec.value;
  }
  function put( Store storage store, address key, string memory value ) internal returns ( bool replaced ) {
    replaced = false;
