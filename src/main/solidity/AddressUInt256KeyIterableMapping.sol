...
library AddressUInt256KeyIterableMapping {
  ...
  function get( Store storage store, address key ) internal view returns ( bool exists, uint256 value ) {
    Record storage rec = store.map[key];
    exists = rec.exists;
    value = rec.value;
  }
  function put( Store storage store, address key, uint256 value ) internal returns ( bool replaced ) {
    replaced = false;
